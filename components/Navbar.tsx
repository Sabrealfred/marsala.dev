  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-300 ${
        isScrolled
          ? "border-b border-moss-200 bg-white/95 shadow-sm backdrop-blur-xl"
          : "bg-transparent"
      }`}
    >
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-6 py-5 lg:px-8">
        <Link href="/" className="flex items-center gap-3">
          ...
          <span className="text-lg font-semibold tracking-tight text-moss-950">
            Marsala
          </span>
        </Link>
          ...
          <ul className="hidden items-center gap-8 text-sm font-medium text-sage-600 md:flex">
            {links.map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href}
                  className="transition-colors duration-200 hover:text-moss-700"
                >
                  {link.label}
                </Link>
              </li>
